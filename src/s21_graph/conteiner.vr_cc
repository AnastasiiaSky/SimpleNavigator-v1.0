...
void DFS(Node node, HashSet<Node> passed) {
  ...
  for (Edge edge : node.edges)
    if (!passed.contains(edge.adjacentNode)) DFS(edge.adjacentNode, passed);
}
...
// создаем структуру данных стек, кидаем туда самый первый узел с которого
// начинается обход и крутим цикл до тех пор пока стек полностью не опустеет
void DFS(Node node, HasgSet<Node> passed) {
  void DFS(Node node, HasgSet<Node> passed) {
    ...
    while (stack.size() != 0) {
      ...
      for (Edge edge : node.edges) {
        if (!passed.contains(
                edge.adjacentNode)) {     // отправим первого найденного ребенка
                                          // которого не посещали
          stack.push(edge.adjacentNode);  // в стек
          hasChildren = true;
          break;
        }
      }  // если дошли до конца (у крайнего узла нет дедей) то удаляем его из
         // стека и поднимается выше
      if (!hasChildren) stack.pop();
    }
  }
}

boolean getPath(Node start, Node end, HashSet<Node> passed,
...
void getPath(Node start, Node end, LinkedHashSet<Node> passed,
             List<LinkedHashSet<Node>> paths) {
  if (start == end) {
  ...
  }  // как только мы прошли всех детей какого то узла то мы удаляем их из
     // списка посещенных
  passed.remove(start);
}
...
// обертка для поиска в ширину которая гарантированно пройдет вширину  даже не
// связанные узлы
...
class PathNode {
 ...
 public
  PathNode parent;
 public
  PathNode(Node node, PathNode parent) {
    this.node = node;
    this.parent = parent;
  }
}
