...
void s21::GraphAlgorithms::DepthFirstSearch(s21_Graph graph, int start_vertex) {
  ...
  std::vector<std::vector<int>> adjacency_list =
      graph.getAdjacencyList();  // получаем список смежных вершин из класса
                                 // Graph

  vertex_stack.push(start_vertex);  // добавляем в стэе стартовую точку с
                                    // которой начинаем обход по графц
  ...
  while (!vertex_stack.empty()) {  // пока стэе не пуст проверяем точки
    ...
    if (CheckVisited(visited_vertices, current_vertex) ==
        false) {  // если текущая точка уже посещена переходим к следующей
                  // итерации цикла
      continue;
    }

    std::vector<int> adjacent_vertices =
        adjacency_list[current_vertex];  // создаем свектор со смежными точками
                                         // для текущей точки
    visited_vertices.push_back(
        current_vertex);  // если не посещена добавляем текущую точку в список
                          // песещенных точек

    for (int it = adjacent_vertices.size() - 1; it >= 0;
         --it) {  // проходим в цикле по вектору смежных вершин
      if (CheckVisited(visited_vertices, adjacent_vertices[it]) ==
          true) {  // и если в векторе смежных вершин имеются еще не посещенные
                   // ранее точки
        vertex_stack.push(adjacent_vertices[it]);  // добавляем их в стэк
        std::cout << "adjacent_vertex in if  " << adjacent_vertices[it]
                  << std::endl;
      }
    }
  }
  PrintResultOfDepthFirstSearch(
      visited_vertices);  // после всех предыдущих манипуляций печатаем
                          // результат
}

bool s21::GraphAlgorithms::CheckVisited(std::list<int> visited_vertices,
                                        int current_vertix) {
...
// std::cout << "adjacent_vertex " << adjacency_list[current_vertex].size() <<
// std::endl;
