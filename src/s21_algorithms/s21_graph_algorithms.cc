...
std::vector<int> s21::GraphAlgorithms::BreadthFirstSearch(s21_Graph graph,
                                                          int start_vertex) {
  if (start_vertex > graph.get_graph_size() || start_vertex < 0) {
    std::vector<int> s21::GraphAlgorithms::BreadthFirstSearch(
        s21_Graph & graph, int start_vertex) {
      if (start_vertex > graph.get_graph_size() || start_vertex <= 0) {
        throw std::length_error("Start vertex is incorrect");
      }
    ...
    /// @brief Метод проверки посещенных точек, в которм происходит итерация
    /// по эллементам вектора посещенных точек и сравнение с текущей точкой.
    ...
    // bool s21::GraphAlgorithms::CheckViseted(std::vector<int>
    // visited_vertices, int current_vertix)
    // {
    ...
    /// 2) Заменяем все "0" максимальным значением 3) В первом цикле путь
    /// проходит через каждую промежуточную вершину V, начиная с положение,
    /// когда нет промежуточной вершины  V = 0 3) Находим минимальное значение
    /// между (Значением между двумя вершина в текущем положение  и Суммой
    /// значений между стартовой точкой и промежуточной V и значением между
    /// промежуточной вершиной V и конечной вершиной 4) Преобразуем
    /// результирующую матрицу: Заменяем максимальное значени е 0

    /// @param graph - текущий граф.
    /// @return std::vector<std::vector<int>> min_distance - результатом
    /// работы метода в виде матрицы смежности является вектор посещенных
    /// точек

    std::vector<std::vector<int>>
    s21::GraphAlgorithms::GetShortestPathsBetweenAllVertices(s21_Graph &
                                                             graph) {
    ...
    s21::TsmResult s21::GraphAlgorithms::SolveTravelingSalesmanProblem(
      ...
      // Создаем результирующую структуру, стартовое расстояние макимально
      // возможное Создаем временный путь {0} Константа коэффициент
      // изчезновения ферамона p = (1 - k), константы a и b

      // Cоздаем переменную = количеству муравьев в колонии, при условии, что
      // у нас Цикл пока все муравье из колоние не пройдут по графу, каждый из
      // своей вершины{ каждый последующий муравей выходит из новой вершины V
      // = Z%size Создаем текущее расстояние D Зануляем временный путь, чтоб
      // каждый новый муравей прокладывал свой собственны или лучше создаем
      // его тут

      // Цикл похода одного муравья из текущей вершины, через все вершин, его
      // путь { Создаем лист вероятности, здесь, чтоб он удалялся после
      // каждого цикла Считаем вероятность прохождения муравья по всем
      // доступным вершинам из текущей, сохраняем в листе Выбираем в какую
      // вершину он пошел из вероятно свободных
      /*int SelectNextVertex (list<int> ProbabilityList) {
        ...
        int sum_probability = ProbabilityList[i];
          for(i <= ProbabilityList.size() && sum_probability <= random_c;
      i++){ sum_probability +=  ProbabilityList[i];
          }
          return  i + 1;
      ...
      //
      //
      // !!!  Рассмотреть случаи, когда у него его маршрут приводит в тупик,
      // тггда надо все зачистить и идти дальше к следующей вершине
      // }
      ...
      // что в результирующей структуре: Перезаписываем стартовую вершину и
      // расстояние Если при заданном графе решение задачи невозможно,
      // выведите ошибку.
      // }
      return TsmResult();
    }
    ...
    // void
    // s21::GraphAlgorithms::PrintResultOfDepthFirstSearch(std::vector<int>
    // visited_vertices)
    // {
    //     for(auto it = visited_vertices.begin(); it !=
    ...
    /// @brief Метод для поиска кратчайшего пути между двумя вершинами в графе
    /// с использованием алгоритма Дейкстры.
    ///
    /// Алгоритм работает следующим образом:
    /// 1. Проверяем корректность входных вершин (vertex1 и vertex2) и
    /// получаем
    ///    размер графа (graphSize).
    /// 2. Инициализируем матрицу смежности (adjacencyMatrix) из объекта
    /// графа.
    /// 3. Преобразуем индексы вершин vertex1 и vertex2 в 0
    /// 4. Создаем массив distances для хранения расстояний от начальной
    /// вершины до
    ///    всех остальных вершин графа. Исходное расстояние до начальной
    ///    вершины устанавливается как 0, а до всех остальных вершин - как
    ///    бесконечность (INF).
    /// 5. Создаем массив visited для отслеживания посещенных вершин. Начально
    /// все
    ...
    /// 11. Перебираем смежные вершины (to) и обновляем расстояния до них,
    /// если
    ///     новое расстояние короче текущего.
    /// 12. Если кратчайший путь до вершины vertex2 равен бесконечности (INF),
    /// то
